...
public interface CategoryApi {
        ...
        @Operation(summary = "Enregistrer une catégorie", description = "Cette méthode permet d'enregistrer ou de modifier une catégorie", responses = {
        ...
        @Operation(summary = "Rechercher une categorie par ID", description = "Cette méthode permet de chercher une categorie par son ID", responses = {
        ...
        @Operation(summary = "Rechercher une categorie par code", description = "Cette méthode permet de chercher une categorie par son code", responses = {
                        @ApiResponse(responseCode = "200", description = "La categorie a été trouvée dans la BD"),
                        @ApiResponse(responseCode = "404", description = "Aucune categorie n'existe dans la BD avec le code fourni")
        })
        public CategoryDto findByCodeCategory(@Parameter(description = "Valeurs acceptées [CAT1, CAT2, CAT3]") @PathVariable("codeCategory") String code);

        @GetMapping(value = APP_ROOT + "/categorie/all", produces = MediaType.APPLICATION_JSON_VALUE)
        @Operation(summary = "Liste des catégories", description = "Cette méthode permet de chercher et renvoyer la liste des catégories existantes dans la BD", responses = {
        ...
        @Operation(summary = "Supprimer une categorie", description = "Cette méthode permet de supprimer une categorie par son ID", responses = {
